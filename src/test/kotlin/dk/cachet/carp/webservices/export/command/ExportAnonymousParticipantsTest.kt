...
class ExportAnonymousParticipantsTest
{
    ...
    {
        private val entry = mockk<Export> {
        ...
        {
            every { services.studyService } returns mockk {
                coEvery { getStudyDetails( any() ) } returns mockk {
                    every { protocolSnapshot } returns null
                }
            }

            val command = ExportAnonymousParticipants(
                entry, payload, services, accountService, resourceExporter, fileUtil
            )
        ...
        fun `should return false if the participant role is not in the protocol`()
        {
            every { services.studyService } returns mockk {
                coEvery { getStudyDetails( any() ) } returns mockk {
                    every { protocolSnapshot } returns mockk {
        ...
        fun `should return false if the amount of accounts is not in the range`()
        {
            every { services.studyService } returns mockk {
                coEvery { getStudyDetails( any() ) } returns mockk {
                    every { protocolSnapshot } returns mockk {
                        every { participantRoles } returns setOf( mockk {
            ...
            val command = ExportAnonymousParticipants(
                entry, payload.copy( amountOfAccounts = 0 ), services, accountService, resourceExporter, fileUtil
            )
            val canExecute = command.canExecute()
