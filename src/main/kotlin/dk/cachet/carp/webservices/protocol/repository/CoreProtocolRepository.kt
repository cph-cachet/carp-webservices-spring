...
): StudyProtocolRepository
{
    ...
    override suspend fun addVersion(protocol: StudyProtocol, version: ProtocolVersion) = runBlocking {
        ...
        if (protocolsStoredWithGivenParams.isEmpty())
        {
            ...
            throw IllegalArgumentException(validationMessages.get("protocol.update.not_found", protocol.id.stringRepresentation, protocol.name))
        }
        else
        {
            protocolsStoredWithGivenParams.forEach {
                ...
                {
                    LOGGER.warn("Protocol with owner ID: {}, name: {} already has a version tag with {}", protocol.ownerId.stringRepresentation, protocol.name, version.tag)
                    throw IllegalArgumentException(validationMessages.get("protocol.update.already_exists", protocol.ownerId.stringRepresentation, protocol.name, version.tag))
                }
            }
        }
     ...
     */
    private fun convertCoreProtocolToWSProtocol(protocol: StudyProtocol, version: ProtocolVersion): Protocol
    {
