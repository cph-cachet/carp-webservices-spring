...
interface DataPointRepository: JpaRepository<DataPoint, String>, JpaSpecificationExecutor<DataPoint>, DataPointRepositoryCustom
{
    ...
    fun findByDeploymentId(deploymentId: String, pageable: Pageable): Page<DataPoint>
    ...
    fun findAllByDeploymentIds(@Param("deploymentIds") deploymentIds: Collection<String>): List<DataPoint>

    /** The [findByDeploymentIdAndCreatedBy] interface to retrieve the data point by [deploymentId] and [createdBy]. */
    fun findByDeploymentIdAndCreatedBy(deploymentId: String, createdBy: String, pageable: Pageable): Page<DataPoint>
    ...
    fun countByDeploymentIdAndCreatedBy(deploymentId: String, createdBy: String): Long

    /** The [getStatistics] interface returns statistical information about the given deployments. */
    @Query(nativeQuery = true,
            value = "select deployment_id as did, count(*) as total, carp_header->'data_format'->>'name' as format, cast(created_at as DATE) as stamp " +
                    "from data_points " +
                    "where deployment_id in (:ids) " +
                    "group by deployment_id, stamp, format"
    )
    fun getStatistics(@Param("ids") deploymentIds: Collection<String>): List<Statistics>
    ...
    override fun save(dataPoint: DataPoint, uploadedFile: MultipartFile?): DataPoint
}
...
interface DataPointRepositoryCustom { fun save(dataPoint: DataPoint, uploadedFile: MultipartFile?): DataPoint }

/**
 * The [DataPointRepositoryImpl] implements the [DataPointRepositoryCustom] to insert the new data points.
 */
@Repository
class DataPointRepositoryImpl(@Lazy private val dataPointRepository: DataPointRepository, private val fileStorage: FileStorageImpl): DataPointRepositoryCustom
{
    /**
